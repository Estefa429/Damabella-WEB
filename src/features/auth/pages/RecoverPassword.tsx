...
export function RecoverPassword() {
  ...
  const handleSubmitEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recoveryCode = generateRecoveryCode(email);
  ...
  const handleSubmitCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (verifyRecoveryCode(email, code)) {
        ...
        showToast('El código es inválido o ha expirado', 'error');
      }
    } catch (error) {
      showToast('Error al verificar el código. Intenta de nuevo.', 'error');
    } finally {
      setLoading(false);
    }
  };
